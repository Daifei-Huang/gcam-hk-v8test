...
module_gcamchina_L100.Socioeconomics_preliminary <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    Region <- province.name <- Year <- GDP <- Population <- Sum <- Difference <- . <- `National Total` <- `Adjusted Population` <-
      ...
      SSP1 <- SSP2 <- SSP3 <- SSP4 <- SSP5 <- `SSP Growth Rate` <- `Growth Rate` <- `SSP National Projection` <-
      GDPperC_currentCNY <- `GDP deflator` <- GDPperC_constant2010CNY <- GDPperCadj <- GDPfinal <- GDP_deflator_CNY_WB <-
      National_GDP_perC <- GDPperCprojections <- GDPperC <- GDPbyCalcSum <- SSP2_HK <- HK_GDP_growth_rate <- NULL # silence package check.
  ...
  # ------------------------------------------------------------------------------------------------------------------------
    ...
    GDP_raw_long %>%
      ...
      left_join( GDP_total_long, by = "Year" ) %>%
      left_join( GDP_sum_long, by = "Year" ) %>%
      left_join( GDP_difference_long, by = "Year" ) -> GDP_2019_long
    ...
    population_long_2024 %>%
      ...
      left_join( GDP_total_long, by = "Year" ) %>%
      left_join( IMF_growth_rates, by = "Year" ) %>%
      left_join( IMF_growth_rates_hk %>% select(Year, IMF_HK = `IMF growth rate`), by = "Year" ) %>%
      mutate(`IMF growth rate` = if_else(province.name == "Hong Kong", IMF_HK, `IMF growth rate`)) %>%
      select(-IMF_HK) %>%
      mutate( "GDP_deflator_CNY_WB" = (( `National Total` / 10 ) *
                                         # ( if_else(province.name != "Hong Kong", GDP_deflator_2010, GDP_deflator_2010_hk) / `GDP deflator` ) ) / CNY_USD_2010_exchange_rate ) -> GDP_deflator_CNY_WB_2024
                                         ( GDP_deflator_2010 / `GDP deflator` ) ) / CNY_USD_2010_exchange_rate ) -> GDP_deflator_CNY_WB_2024
    # *** for HK version *** // Daifei 28/10/2025
    ...
    province_2110 %>%
      ...
      # *** for HK version *** // Daifei 03/07/2026
      left_join( SSP_gdp_HK %>%
                   select( Year, SSP2_HK = SSP2 ), by = "Year" ) %>%
      mutate( `SSP Growth Rate` = if_else( province.name == "Hong Kong", SSP2_HK, `SSP Growth Rate` ) / 100 ) %>%
      select( -SSP2_HK ) %>%
      # *** for HK version *** // Daifei 03/07/2026
    ...
    # *** for HK version *** // Daifei 05/07/2026 Hong Kong SSP2 growth rate for GDP
    # ponytail: HK follows the annual SSP2 GDP path; other provinces keep the legacy endpoint-smoothed rate.
    SSP_gdp_HK %>%
      select( Year, HK_GDP_growth_rate = SSP2 ) %>%
      mutate( HK_GDP_growth_rate = HK_GDP_growth_rate / 100 ) -> hk_gdp_growth_rate

    growth_rate_calc %>%
      mutate( "Year" = "2020",
              Year = as.numeric( Year ) ) %>%
      left_join( hk_gdp_growth_rate, by = "Year" ) %>%
      mutate( `Growth Rate` = if_else( province.name == "Hong Kong" & !is.na( HK_GDP_growth_rate ),
                                       HK_GDP_growth_rate,
                                       `Growth Rate` ),
              "GDPperCprojections" = GDPperCadj_2019 * ( 1 + `Growth Rate` ) ) %>%
      select( -c( GDPperCadj_2019, National_GDP_perC_2110, `Growth Rate`, HK_GDP_growth_rate ) ) -> GDPperCprojections_2020
    # *** for HK version *** // Daifei 05/07/2026 Hong Kong SSP2 growth rate for GDP
