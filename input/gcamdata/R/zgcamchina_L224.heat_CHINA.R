...
module_gcamchina_L224_China.heat <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L2231.StubTechProd_coal_vintage_CHINA <-  get_data(all_data,"L2231.StubTechProd_coal_vintage_CHINA", strip_attributes = T)
    ...
    A24.sector %>%
      ...
      write_to_all_provinces(c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), gcamchina.PROVINCES_NOMC) ->
    ...
    A24.subsector_logit %>%
      ...
      write_to_all_provinces(c(LEVEL2_DATA_NAMES[["SubsectorLogit"]], LOGIT_TYPE_COLNAME), gcamchina.PROVINCES_NOMC) ->
      L224.SubsectorLogit_heat_china
    # *** for HK version *** //
    ...
    if(any(!is.na(A24.subsector_shrwt$year))) {
      ...
      A24.subsector_shrwt %>%
        ...
        write_to_all_provinces(LEVEL2_DATA_NAMES[["SubsectorShrwt"]], gcamchina.PROVINCES_NOMC ) ->
        L224.SubsectorShrwt_heat_china
      # *** for HK version *** //
    }
    ...
    if(any(!is.na(A24.subsector_shrwt$year.fillout))) {
      ...
      A24.subsector_shrwt %>%
        ...
        write_to_all_provinces(LEVEL2_DATA_NAMES[["SubsectorShrwtFllt"]], gcamchina.PROVINCES_NOMC ) ->
        L224.SubsectorShrwtFllt_heat_china
      # *** for HK version *** //
    }
    ...
    L124.in_EJ_R_heat_F_Yh %>%
      ...
      write_to_all_provinces(LEVEL2_DATA_NAMES[["StubTechCalInput"]], gcamchina.PROVINCES_NOMC ) %>% # *** for HK version *** //
    ...
    L124.out_EJ_R_heatfromelec_F_Yh %>%
      ...
      write_to_all_provinces(c('region','fuel','sector','year','value'), gcamchina.PROVINCES_NOMC ) %>% # *** for HK version *** //
      left_join(heat_en_by_province %>% rename(region = province) ,by = c("region","year")) %>%
    ...
    # Yang Ou Dec 2023
    # 2010/2015/2021 Fuel Adjustment Note:
    ...
    # Updated to also include 2015 and 2021 where similar issues were identified.

    no_gas_tech_years <- L2234.StubTechProd_elecS_CHINA %>%
      ...
      ungroup() %>%
      filter(no_gas < 0.1) %>%
      mutate(region_year = paste(region, year, sep = "_"))

    L124.out_EJ_province_heatfromelec_F_Yh %>%
      mutate(region_year = paste(region, year, sep = "_")) %>%
      mutate(fuel_new = if_else( (region_year %in% no_gas_tech_years$region_year) & fuel == "gas", "coal", fuel)) %>%
      select(-region_year) %>%
    ...
    if(any(is.na(A24.subsector_interp$to.value))) {
      ...
      A24.subsector_interp %>%
        ...
        write_to_all_provinces(LEVEL2_DATA_NAMES[["SubsectorInterp"]], gcamchina.PROVINCES_NOMC ) ->
        L224.SubsectorInterp_heat_china
      # *** for HK version *** //
    }
    ...
    if(any(!is.na(A24.subsector_interp$to.value))) {
      ...
      A24.subsector_interp %>%
        ...
        write_to_all_provinces(LEVEL2_DATA_NAMES[["SubsectorInterpTo"]], gcamchina.PROVINCES_NOMC ) ->
        L224.SubsectorInterpTo_heat_china
      # *** for HK version *** //
    }
    ...
    L224.StubTechSecOut_elec_china_no_gas<-L224.StubTechSecOut_elec_china %>%
      filter(subsector != "gas")
    L224.StubTechSecOut_elec_china_gas<-L224.StubTechSecOut_elec_china %>%
      filter(subsector == "gas")
    ...
    L224.StubTechSecOut_elec_china_no_gas %>%
      ...
      mutate( `coal_base_conv pul 1991-1995` = case_when(stub.technology == "coal_base_conv pul" ~ 1),
              `coal_base_conv pul 1996-2000` = case_when(stub.technology == "coal_base_conv pul" ~ 1),
              ...
              `coal_int_conv pul 1996-2000` = case_when(stub.technology == "coal_int_conv pul" ~ 1),
              ...
              `coal_peak_conv pul 1996-2000` = case_when(stub.technology == "coal_peak_conv pul" ~ 1),
              ...
              `coal_subpeak_conv pul 1996-2000` = case_when(stub.technology == "coal_subpeak_conv pul" ~ 1),
      ...
      gather(key = "stub.technology.new", value = TechActive, colnames(.)[grepl("coal_", colnames(.))]) %>%
      filter(!is.na(TechActive)) %>%
      mutate(output.ratio  = output.ratio * TechActive,
             stub.technology = stub.technology.new) %>%
      select(-stub.technology.new, -TechActive) ->
      L224.StubTechSecOut_elec_china_no_gas_processed

    L224.StubTechSecOut_elec_china_no_gas_processed<-L224.StubTechSecOut_elec_china_no_gas_processed %>%
      select("region", "supplysector", "subsector", "stub.technology", "year", "secondary.output", "output.ratio")

    L224.StubTechSecOut_elec_china <- bind_rows(L224.StubTechSecOut_elec_china_no_gas_processed, L224.StubTechSecOut_elec_china_gas)
    ...
    L1231.eff_R_elec_F_tech_Yh %>%
      ...
      write_to_all_provinces(c('region','sector','fuel','technology','year','efficiency'), gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
      filter(fuel == "gas") %>%
