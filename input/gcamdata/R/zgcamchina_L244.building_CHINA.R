  ...
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "energy/A44.gcam_consumer",
             ...
             FILE = "gcam-china/A44.globaltech_retirement",
              ...
              FILE = "gcam-china/A44.demand_satiation_mult_HK", # *** for HK version *** // Hong Kong-specific building service satiation multipliers.
             ...
             FILE = "gcam-china/A44.fuelprefElasticity",
              ...
              FILE = "gcam-china/HDDCDD_constdd_noGCM_HK", # *** for HK version *** // Hong Kong HDD/CDD using customized base temperature.
    ...
    return(c("L244.DeleteConsumer_CHINAbld",
             ...
             "L244.ThermalBaseService_gcamchina",
              ...
              "L244.ThermalServiceSatiation_gcamchina_HK", # *** for HK version *** // Hong Kong-specific thermal service satiation.
              "L244.GenericServiceSatiation_gcamchina_HK", # *** for HK version *** // Hong Kong-specific generic service satiation.
             ...
             "L244.DeleteGenericService_gcamchina",
              ...
              "L244.DeleteConsumer_HKbld", # *** for HK version *** // Delete HK residential-rural building consumer in XML.
              "L244.DeleteSupplysector_HKbld" # *** for HK version *** // Delete HK residential-rural building supply sector in XML.
    ))
    ...
    A44.demand_satiation_mult_HK <- get_data(all_data, "gcam-china/A44.demand_satiation_mult_HK", strip_attributes = TRUE) # *** for HK version *** // HK service satiation multipliers.
    ...
    HDDCDD_constdd_noGCM_HK <- get_data(all_data, "gcam-china/HDDCDD_constdd_noGCM_HK", strip_attributes = TRUE) # *** for HK version *** // HK HDD/CDD data with customized base temperature.
    ...
    L244.gcamchina_consumer_grp_resid <- write_to_all_provinces(A44.gcam_consumer, c("region", "gcam.consumer"), gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Hong Kong has no residential-rural building population/income share.
    L244.SubregionalShares_gcamchina0 <- L244.SubregionalShares_gcamchina0 %>%
      mutate(share = if_else(region == "HK" & gcam.consumer == "resid_rural", 0, share))
    # *** for HK version *** //
    ...
    L244.SubregionalShares_gcamchina <- L244.SubregionalShares_gcamchina00 %>%
      ...
      bind_rows(write_to_all_provinces(A44.gcam_consumer, c("region", "gcam.consumer"), gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
                  filter(gcam.consumer == "comm") %>%
                  repeat_add_columns(tibble(pop.year.fillout=MODEL_YEARS)) %>%
                  mutate(inc.year.fillout = pop.year.fillout) %>%
    ...
    # *** for HK version *** //
    # Keep historical HK residential-rural income shares at zero to match the deleted HK rural building consumer.
    L244.SubregionalShares_gcamchina_allhist0 <- L244.SubregionalShares_gcamchina_allhist0 %>%
      mutate(share = if_else(region == "HK" & gcam.consumer == "resid_rural", 0, share))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Avoid undefined HK residential-rural GDP per capita after the rural consumer is removed.
    L101.pcgdp_thous90USD_province_Yh_gr <- L101.pcgdp_thous90USD_province_Yh_gr %>%
      mutate(pcGDP_thous90USD = if_else(province == "HK" & grepl("resid_rural", gcam.consumer), 0, pcGDP_thous90USD))
    # *** for HK version *** //
    ...
    L244.DemandFunction_flsp_gcamchina <- write_to_all_provinces(A44.demandFn_flsp, LEVEL2_DATA_NAMES[["DemandFunction_flsp"]], gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
      filter(grepl('resid', gcam.consumer)) %>%
      repeat_add_columns(tibble(group=unique(L144.income_shares$group))) %>%
      unite(gcam.consumer, c(gcam.consumer,group),sep="_") %>%
      # bind_rows(write_to_all_states(A44.demandFn_flsp, LEVEL2_DATA_NAMES[["DemandFunction_flsp"]], gcamchina.PROVINCES_NOHKMC) %>%
      bind_rows(write_to_all_states(A44.demandFn_flsp, LEVEL2_DATA_NAMES[["DemandFunction_flsp"]], gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
                  filter(gcam.consumer == "comm"))
    ...
    L244.DemandFunction_serv_gcamchina <- write_to_all_provinces(A44.demandFn_serv, LEVEL2_DATA_NAMES[["DemandFunction_serv"]], gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
      filter(grepl('resid', gcam.consumer)) %>%
      repeat_add_columns(tibble(group=unique(L144.income_shares$group))) %>%
      unite(gcam.consumer, c(gcam.consumer,group),sep="_") %>%
      # bind_rows(write_to_all_provinces(A44.demandFn_serv, LEVEL2_DATA_NAMES[["DemandFunction_serv"]], gcamchina.PROVINCES_NOHKMC)
      bind_rows(write_to_all_provinces(A44.demandFn_serv, LEVEL2_DATA_NAMES[["DemandFunction_serv"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
                %>% filter(gcam.consumer == "comm"))
    ...
    L244.PriceExp_IntGains_gcamchina <- write_to_all_provinces(A44.gcam_consumer, LEVEL2_DATA_NAMES[["PriceExp_IntGains"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    L144.flsp_param_china <- L144.flsp_param %>%
      ...
      repeat_add_columns(tibble(region=gcamchina.PROVINCES_NOMC)) # *** for HK version *** //
    ...
    L144.hab_land_flsp_china_fin <- L144.hab_land_flsp_china %>%
      ...
      filter(province %in% gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
    ...
    # *** for HK version *** //
    # HK residential-rural population is zero, so force base per-capita floorspace to zero instead of carrying NaN.
    L244.Base_pcFlsp <- L244.Base_pcFlsp %>%
      mutate(base.pcFlsp = if_else(region == "HK" & nodeInput == "resid_rural", 0, base.pcFlsp))
    # *** for HK version *** //
    ...
    L143.HDDCDD_scen_province <- L143.HDDCDD_scen_R_Y %>%
      ...
      repeat_add_columns(tibble(province=paste0(gcamchina.PROVINCES_NOMC))) %>% # *** for HK version *** //
      select(-GCAM_region_ID)
    ...
    # *** for HK version *** //
    # Replace Hong Kong constdd/no_GCM values after 2000 with HK HDD/CDD data using customized base temperature.
    L244.HDDCDD_scen_province_HKconstdd21st <- L244.HDDCDD_scen_province %>%
      filter(Scen == "constdd", region == "HK", year > 2000)
    L244.HDDCDD_scen_province <- L244.HDDCDD_scen_province %>%
      anti_join(L244.HDDCDD_scen_province_HKconstdd21st, by = c("Scen", "region", "year")) %>%
      bind_rows(HDDCDD_constdd_noGCM_HK)
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Use the HK customized constdd/no_GCM series for Hong Kong's climate normal.
    L244.HDDCDD_normal_province <- L244.HDDCDD_normal_province %>%
      filter(region != "HK") %>%
      bind_rows(L244.HDDCDD_scen_province %>%
                  filter(year %in% seq(1981, 2000),
                         region == "HK", Scen == "constdd") %>%
                  group_by(region, variable) %>%
                  summarise(degree.days = mean(degree.days)) %>%
                  ungroup())
    # *** for HK version *** //
    ...
    L244.Supplysector_bld_gcamchina <- write_to_all_provinces(A44.sector, c(LEVEL2_DATA_NAMES[["Supplysector"]], LOGIT_TYPE_COLNAME), gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    L244.FinalEnergyKeyword_bld_gcamchina <- write_to_all_provinces(A44.sector, LEVEL2_DATA_NAMES[["FinalEnergyKeyword"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    L244.SubsectorLogit_bld_gcamchina <- write_to_all_provinces(A44.subsector_logit, c(LEVEL2_DATA_NAMES[["SubsectorLogit"]], LOGIT_TYPE_COLNAME), gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    L244.FuelPrefElast_bld_gcamchina <- A44.fuelprefElasticity %>%
      ...
      write_to_all_provinces(LEVEL2_DATA_NAMES[["FuelPrefElast"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    if(any(!is.na(A44.subsector_shrwt$year))) {
      L244.SubsectorShrwt_bld_gcamchina <- write_to_all_provinces(A44.subsector_shrwt %>%
                                                                    ...
                                                                    filter(!is.na(year)), LEVEL2_DATA_NAMES[["SubsectorShrwt"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    }
    if(any(!is.na(A44.subsector_shrwt$year.fillout))) {
      L244.SubsectorShrwtFllt_bld_gcamchina <- write_to_all_provinces(A44.subsector_shrwt %>%
                                                                        ...
                                                                        filter(!is.na(year.fillout)), LEVEL2_DATA_NAMES[["SubsectorShrwtFllt"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    }
    ...
    if(any(is.na(A44.subsector_interp$to.value))) {
      L244.SubsectorInterp_bld_gcamchina <- write_to_all_provinces(A44.subsector_interp %>%
                                                                     ...
                                                                     filter(is.na(to.value)), LEVEL2_DATA_NAMES[["SubsectorInterp"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    }
    if(any(!is.na(A44.subsector_interp$to.value))) {
      L244.SubsectorInterpTo_bld_gcamchina <- write_to_all_provinces(A44.subsector_interp %>%
                                                                       ...
                                                                       filter(!is.na(to.value)), LEVEL2_DATA_NAMES[["SubsectorInterpTo"]], gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    }
    ...
    L244.StubTech_bld_gcamchina <- add.cg(A44.globaltech_eff) %>%
      ...
      write_to_all_provinces(LEVEL2_DATA_NAMES[["Tech"]], gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
    ...
    L244.StubTechMarket_bld_gcamchina <- add.cg(L244.end_use_eff) %>%
      ...
      write_to_all_provinces(LEVEL2_DATA_NAMES[["StubTechMarket"]], gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Hong Kong building service demand is close to 2015 saturation; compute a separate HK-specific
    # generic-service satiation using the HK multiplier and keep it for a separate XML.
    L244.GenericServiceSatiation_gcamchina_HK <- L244.GenericBaseService_gcamchina %>%
      filter(year == max(MODEL_BASE_YEARS), region == "HK") %>%
      left_join_error_no_match(L244.Floorspace_gcamchina %>%
                                 filter(region == "HK") %>%
                                 group_by(region,nodeInput,building.node.input,year) %>%
                                 summarise(base.building.size=sum(base.building.size)) %>%
                                 ...
                                 mutate(gcam.consumer = nodeInput),
                               by = c(LEVEL2_DATA_NAMES[["BldNodes"]], "year")) %>%
      left_join_error_no_match(A44.demand_satiation_mult_HK %>% select(supplysector, multiplier.HK = multiplier),
    ...
    # *** for HK version *** //
    # Compute Hong Kong-specific thermal-service satiation using the HK multiplier and keep it
    # separate from the default China building XML.
    L244.ThermalServiceSatiation_gcamchina_HK <- L244.ThermalBaseService_gcamchina %>%
      filter(year == max(MODEL_BASE_YEARS), region == "HK") %>%
      left_join_error_no_match(L244.Floorspace_gcamchina %>%
                                 filter(region == "HK") %>%
                                 group_by(region,nodeInput,building.node.input,year) %>%
    ...
    # *** for HK version *** //
    # Remove HK residential-rural generic service share before normalizing shares.
    L244.GenericShares_gcamchina_pre <- L244.GenericShares_gcamchina_pre %>%
      mutate(serv = if_else(region == "HK" & nodeInput == "resid_rural", 0, serv))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Apply the same residential income-group service naming to the HK-specific thermal satiation output.
    L244.ThermalServiceSatiation_gcamchina_HK <- L244.ThermalServiceSatiation_gcamchina_HK %>%
      filter(grepl("resid",gcam.consumer)) %>%
      separate(gcam.consumer,c("adj","group"), sep = "_(?=[^_]*$)",remove = F) %>%
      mutate(thermal.building.service.input = paste0(thermal.building.service.input,"_",group)) %>%
      select(LEVEL2_DATA_NAMES[["ThermalServiceSatiation"]]) %>%
      bind_rows(L244.ThermalServiceSatiation_gcamchina_HK %>%
                  filter(grepl("comm",gcam.consumer)))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Apply the same residential income-group service naming to the HK-specific generic satiation output.
    L244.GenericServiceSatiation_gcamchina_HK <- L244.GenericServiceSatiation_gcamchina_HK %>%
      ungroup() %>%
      filter(grepl("resid",gcam.consumer)) %>%
      separate(gcam.consumer,c("adj","group"), sep = "_(?=[^_]*$)",remove = F) %>%
      mutate(building.service.input = paste0(building.service.input,"_",group)) %>%
      select(LEVEL2_DATA_NAMES[["GenericServiceSatiation"]]) %>%
      bind_rows(L244.GenericServiceSatiation_gcamchina_HK %>%
                  filter(grepl("comm",gcam.consumer)))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Delete Hong Kong residential-rural building consumer and supplysector in a separate XML,
    # because Hong Kong has no residential-rural building service demand.
    L244.DeleteConsumer_HKbld <- tibble(region = "HK",
                                        gcam.consumer = unique(L244.DemandFunction_serv_gcamchina$gcam.consumer)) %>%
      filter(grepl('resid_rural', gcam.consumer))

    L244.DeleteSupplysector_HKbld <- tibble(region = "HK",
                                            supplysector = unique(L244.Supplysector_bld_gcamchina$supplysector)) %>%
      filter(grepl('resid_rural', supplysector))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Metadata for the HK-specific building service satiation XML inputs.
    L244.ThermalServiceSatiation_gcamchina_HK %>%
      add_title("Satiation levels assumed for thermal building services in Hong Kong") %>%
      add_units("EJ/billion m2 floorspace") %>%
      add_comments("Satiation level = base service / floorspace * Hong Kong exogenous multiplier") %>%
      add_legacy_name("L244.ThermalServiceSatiation_HK") %>%
      add_precursors("L144.in_EJ_province_bld_F_U", "gcam-china/calibrated_techs_bld_china",
                     "gcam-china/A44.globaltech_eff", "gcam-china/A44.globaltech_eff_avg", "gcam-china/A44.globaltech_shares",
                     "gcam-china/A44.gcam_consumer", "L144.flsp_bm2_province_bld",
                     "gcam-china/A44.demand_satiation_mult_HK") ->
      L244.ThermalServiceSatiation_gcamchina_HK

    L244.GenericServiceSatiation_gcamchina_HK %>%
      add_title("Satiation levels assumed for non-thermal building services in Hong Kong") %>%
      add_units("EJ/billion m2 floorspace") %>%
      add_comments("Satiation level = base service / floorspace * Hong Kong exogenous multiplier") %>%
      add_legacy_name("L244.GenericServiceSatiation_HK") %>%
      add_precursors("L144.in_EJ_province_bld_F_U", "gcam-china/calibrated_techs_bld_china",
                     "gcam-china/A44.globaltech_eff", "gcam-china/A44.globaltech_eff_avg", "gcam-china/A44.globaltech_shares",
                     "gcam-china/A44.gcam_consumer", "L144.flsp_bm2_province_bld",
                     "gcam-china/A44.demand_satiation_mult_HK") ->
    ...
    L244.HDDCDD_A2_CCSM3x_China %>%
      ...
      add_precursors("L143.HDDCDD_scen_R_Y", "gcam-china/A44.sector",
                     "gcam-china/HDDCDD_constdd_noGCM_HK", # *** for HK version *** // Hong Kong HDD/CDD replacement data.
    ...
    # *** for HK version *** //
    # Metadata for deleting Hong Kong residential-rural building entries in a separate XML.
    L244.DeleteConsumer_HKbld %>%
      add_title("Deletes building sector's residential-rural consumer in Hong Kong") %>%
      add_units("NA") %>%
      add_comments("Hong Kong has no residential-rural building consumer") %>%
      add_legacy_name("L244.DeleteConsumer_HKbld") %>%
      add_precursors("energy/A44.gcam_consumer","socioeconomics/income_shares") ->
      L244.DeleteConsumer_HKbld

    L244.DeleteSupplysector_HKbld %>%
      add_title("Deletes building sector's residential-rural supplysector in Hong Kong") %>%
      add_units("NA") %>%
      add_comments("Hong Kong has no residential-rural building supplysector") %>%
      add_legacy_name("L244.DeleteSupplysector_HKbld") %>%
      add_precursors("energy/A44.sector","L244.Supplysector_bld","socioeconomics/income_shares") ->
      L244.DeleteSupplysector_HKbld
    # *** for HK version *** //
    ...
    return_data(L244.DeleteConsumer_CHINAbld,
                ...
                L244.ThermalShares_gcamchina,
                L244.FuelPrefElast_bld_gcamchina,
                L244.DeleteConsumer_HKbld, # *** for HK version *** // HK residential-rural consumer deletion.
                L244.DeleteSupplysector_HKbld # *** for HK version *** // HK residential-rural supplysector deletion.
    )
  } else {
