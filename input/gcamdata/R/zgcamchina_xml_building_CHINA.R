...
#' the generated outputs: \code{building_CHINA.xml}, \code{building_CHINA_deleteHKrural.xml},
#' and \code{building_CHINA_HKsatiation.xml}. The corresponding file in the
  ...
  if(command == driver.DECLARE_INPUTS) {
    return(c("L244.DeleteConsumer_CHINAbld",
             ...
             "L244.ThermalBaseDens_gcamchina",
              ...
              "L244.DeleteConsumer_HKbld", # *** for HK version *** // HK residential-rural building consumer deletion.
              "L244.DeleteSupplysector_HKbld", # *** for HK version *** // HK residential-rural building supplysector deletion.
              "L244.ThermalServiceSatiation_gcamchina_HK", # *** for HK version *** // HK thermal service satiation.
              "L244.GenericServiceSatiation_gcamchina_HK" # *** for HK version *** // HK generic service satiation.
              ))
  } else if(command == driver.DECLARE_OUTPUTS) {
    return(c(XML = "building_CHINA.xml",
             XML = "building_CHINA_deleteHKrural.xml", # *** for HK version *** // Deletes HK residential-rural building sector.
             XML = "building_CHINA_HKsatiation.xml" # *** for HK version *** // Adds HK-specific service satiation.
             ))
    ...
    # *** for HK version *** //
    # Load HK-only building outputs generated in L244 for separate XML files.
    L244.DeleteConsumer_HKbld <- get_data(all_data, "L244.DeleteConsumer_HKbld")
    L244.DeleteSupplysector_HKbld <- get_data(all_data, "L244.DeleteSupplysector_HKbld")
    L244.ThermalServiceSatiation_gcamchina_HK <- get_data(all_data, "L244.ThermalServiceSatiation_gcamchina_HK")
    L244.GenericServiceSatiation_gcamchina_HK <- get_data(all_data, "L244.GenericServiceSatiation_gcamchina_HK")
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Separate XML for deleting Hong Kong residential-rural building entries.
    create_xml("building_CHINA_deleteHKrural.xml") %>%
      add_xml_data(L244.DeleteConsumer_HKbld, "DeleteConsumer") %>%
      add_xml_data(L244.DeleteSupplysector_HKbld, "DeleteSupplysector") %>%
      add_precursors("L244.DeleteConsumer_HKbld",
                     "L244.DeleteSupplysector_HKbld") ->
      building_CHINA_deleteHKrural.xml

    # Separate XML for Hong Kong-specific building service satiation assumptions.
    create_xml("building_CHINA_HKsatiation.xml") %>%
      add_xml_data(L244.ThermalServiceSatiation_gcamchina_HK, "ThermalServiceSatiation") %>%
      add_xml_data(L244.GenericServiceSatiation_gcamchina_HK, "GenericServiceSatiation") %>%
      add_precursors("L244.ThermalServiceSatiation_gcamchina_HK",
                     "L244.GenericServiceSatiation_gcamchina_HK") ->
      building_CHINA_HKsatiation.xml
    # *** for HK version *** //

    return_data(building_CHINA.xml,
                building_CHINA_deleteHKrural.xml, # *** for HK version *** // HK residential-rural deletion XML.
                building_CHINA_HKsatiation.xml # *** for HK version *** // HK service satiation XML.
                )
  } else {
