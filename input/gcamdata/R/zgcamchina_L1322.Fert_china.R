...
module_gcamchina_L1322.Fert <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    ...
    L1322.IO_R_Fert_F_Yh <- get_data(all_data, "L1322.IO_R_Fert_F_Yh", strip_attributes = T)
    ...
    L1322.in_Mtce_province_Fert %>%
      ...
      mutate(multiplier = approx_fun(year, multiplier, rule = 2), sector = "ammonia") %>%
      ungroup() %>%
      select(-value, -EBProcess, -EBMaterial, -fuel) %>%
      # Use the same shares for all fuels
      repeat_add_columns(tibble(fuel = unique(L1322.Fert_Prod_MtNH3_R_F_Y$fuel))) ->
      L1322.in_pct_province_Fert_repF

    # Approtion national production to provinces use "Coal Raw" in Agriculture as a proxy
    L1322.in_pct_province_Fert_repF %>%
      left_join_error_no_match(filter(L1322.Fert_Prod_MtNH3_R_F_Y, GCAM_region_ID == gcamchina.REGION_ID), by = c("fuel", "sector", "year")) %>%
    ...
    L1322.IO_R_Fert_F_Yh %>%
      ...
      repeat_add_columns(tibble(province = gcamchina.PROVINCES_NOMC)) %>% # *** for HK version *** //
      filter(province != 'XZ') %>%
    ...
    L1322.out_Mt_province_Fert_Yh %>%
      ...
      add_precursors("L1322.Fert_Prod_MtNH3_R_F_Y", "L101.NBS_use_all_Mtce", "L1322.IO_R_Fert_F_Yh") ->
      L1322.out_Mt_province_Fert_Yh
