...
module_gcamchina_L144.Building <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c("L142.in_EJ_R_bld_F_Yh",
             ...
             FILE="gcam-china/cR_BldS_F_U_share_res_HK", # *** for HK version *** // Hong Kong-customized residential service shares.
             FILE="gcam-china/cR_BldS_F_U_share_com_HK", # *** for HK version *** // Hong Kong-customized commercial service shares.
    ...
    # *** for HK version *** //
    # Use Hong Kong-specific service shares to split HK residential and commercial building fuel by service.
    cR_BldS_F_U_share_res_HK <- get_data(all_data, "gcam-china/cR_BldS_F_U_share_res_HK", strip_attributes = TRUE) # *** for HK version *** // HK residential service shares.
    cR_BldS_F_U_share_com_HK <- get_data(all_data, "gcam-china/cR_BldS_F_U_share_com_HK", strip_attributes = TRUE) # *** for HK version *** // HK commercial service shares.
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Reshape Hong Kong service-share tables in the same format as the mainland share tables.
    ...
    L144.in_EJ_province_bld_F_U_res <- L144.cR_S_F_U_share_res %>%
      ...
      filter(province != "HK") %>% # *** for HK version *** // HK residential services are allocated with HK-customized shares below.
    ...
    # *** for HK version *** //
    # Apply Hong Kong-customized residential service shares; HK has no residential rural service demand.
    L144.in_EJ_province_bld_F_U_res_HK <- L144.cR_S_F_U_share_res_HK %>%
      left_join(province_names_mappings, by = c("climate.region")) %>%
      mutate(share = if_else(province == "HK" & sector == "resid_rural", 0, share)) %>%
      select(climate.region, sector, fuel, service, share, province) %>%
      left_join(L144.in_EJ_province_bld_F %>%
                  filter(grepl('resid', sector), province == "HK"), by = c("province", "sector", "fuel")) %>%
      mutate(value = value * share,
             value = if_else(is.na(value), 0, value),
             service = paste(sector, service, sep = " ")) %>%
      select(province, sector, fuel, service, year, value) %>%
      na.omit()
    # *** for HK version *** //
    ...
    L144.in_EJ_province_bld_F_U_com <- L144.cR_S_F_U_share_com %>%
      # must use left join, as number of rows is changing
      left_join(province_names_mappings, by = c("climate.region")) %>%
      filter(province != "HK") %>% # *** for HK version *** // HK commercial services are allocated with HK-customized shares below.
      select(climate.region, sector, fuel, service, share, province) %>%
      # must use left join, as number of rows is changing
      left_join(L144.in_EJ_province_bld_F %>%
    ...
    # *** for HK version *** //
    # Apply Hong Kong-customized commercial service shares.
    L144.in_EJ_province_bld_F_U_com_HK <- L144.cR_S_F_U_share_com_HK %>%
      left_join(province_names_mappings, by = c("climate.region")) %>%
      filter(province == "HK") %>%
      select(climate.region, sector, fuel, service, share, province) %>%
      left_join(L144.in_EJ_province_bld_F %>%
                  filter(grepl('comm', sector), province == "HK"), by = c("province", "sector", "fuel")) %>%
      mutate(value = value * share,
             value = if_else(is.na(value), 0, value),
             service = paste(sector, service, sep = " ")) %>%
      select(province, sector, fuel, service, year, value) %>%
    ...
    L144.in_EJ_province_bld_F_U_pre <- L144.in_EJ_province_bld_F_U_res %>%
      bind_rows(L144.in_EJ_province_bld_F_U_res_HK) %>% # *** for HK version *** // Add HK residential service allocation.
      bind_rows(L144.in_EJ_province_bld_F_U_com) %>%
      bind_rows(L144.in_EJ_province_bld_F_U_com_HK) # *** for HK version *** // Add HK commercial service allocation.
    ...
    # *** for HK version *** //
    # Hong Kong commercial buildings have distinct historical energy intensity; adjust the share before allocating China commercial floorspace.
    L144.in_pct_province_bld_comm <- L144.in_pct_province_bld_comm %>%
      mutate(value = if_else(province == "HK" & year %in% 1981:2003, value * 0.3,
                             if_else(province == "HK" & year %in% 2004:2007, value * 0.46,
                                     if_else(province == "HK" & year %in% 2008:2015, value * 0.9331, value))))
    # *** for HK version *** //
    ...
    # *** for HK version *** //
    # Hong Kong has no residential-rural building floorspace.
    L144.flsp_bm2_prov_res_rural <- L144.flsp_bm2_prov_res_rural %>%
      mutate(value = if_else(province == "HK" & sector == "resid_rural", 0, value))
    # *** for HK version *** //
    ...
    L144.in_EJ_province_bld_F_U %>%
      ...
      add_precursors("L142.in_EJ_R_bld_F_Yh",
                     FILE="gcam-china/province_names_mappings",
                      ...
                      FILE="gcam-china/cR_BldS_F_U_share_res",
                      FILE="gcam-china/cR_BldS_F_U_share_com",
                      FILE="gcam-china/cR_BldS_F_U_share_res_HK", # *** for HK version *** // Hong Kong residential service shares.
                      FILE="gcam-china/cR_BldS_F_U_share_com_HK", # *** for HK version *** // Hong Kong commercial service shares.
                      "L101.inNBS_Mtce_province_S_F")  ->
      L144.in_EJ_province_bld_F_U
