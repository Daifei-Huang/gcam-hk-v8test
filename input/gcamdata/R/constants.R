...
CONV_EJ_MTCE <- 34.12                   #EJ to Metric tons of coal equivalent 0821 by xsl
CONV_BCM_EJ <- 0.04                     #billion m^3 to EJ 0821 by xsl
...
# *** for HK version *** +
gcamchina.PROVINCES_NOMC <- c("AH", "BJ", "CQ", "FJ", "GD", "GS", "GX", "GZ", "HA", "HB", "HE", "HI", "HK",
...
gcamchina.DIGITS_TRNUSA_DEFAULT     <- 1    # Reduce rounding in detailed CHINA transport for compatability with model
gcamchina.DIGITS_EMISSIONS          <- 5
...
gcamchina.FERT_NAME       <- "ammonia" # Define GCAM-China category name of fertilizer
...
# Electricity load segments
gcamchina.LOAD_SEG_CAL_YEARS <- c(2021, 2015,2010, 2005, 1990)       # Years for which electricity load segments are calibrated
...
gcamchina.BASE_HDD_CHINA <- 4410.838 # zhou. et al(2014)
gcamchina.BASE_CDD_CHINA <- 1549.303 # zhou. et al(2014)
