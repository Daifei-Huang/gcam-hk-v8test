...
module_gcamchina_L100.Socioeconomics_preliminary <- function(command, ...) {
  if(command == driver.DECLARE_INPUTS) {
    return(c(FILE = "gcam-china/China_Compendium_of_Statistics",
             ...
             FILE = "gcam-china/GDP_deflator_HK", # *** for HK version *** // Hong Kong uses its own GDP deflator series.
             FILE = "gcam-china/IMF_growth_rates_HK", # *** for HK version *** // Hong Kong uses its own short-term IMF GDP growth rates.
    ...
    # *** for HK version *** //
    # Load Hong Kong-specific GDP deflator and IMF growth data instead of reusing mainland China values for HK.
    GDP_deflator_raw_hk <- get_data(all_data, "gcam-china/GDP_deflator_HK" ) # *** for HK version *** // HK GDP deflator; https://data.worldbank.org/indicator/NY.GDP.DEFL.ZS?locations=HK
    IMF_growth_rates_hk <- get_data(all_data, "gcam-china/IMF_growth_rates_HK" ) # *** for HK version *** // HK IMF short-term GDP growth rates.
    # *** for HK version *** //
  ...
  # ------------------------------------------------------------------------------------------------------------------------
    ...
    GDP_deflator_2010_hk <- 85.64778605 # *** for HK version *** // Hong Kong GDP deflator value in 2010.
    ...
    # *** for HK version *** //
    # Keep a separate deflator table for Hong Kong historical GDP conversion.
    GDP_deflator_raw_hk %>%
    ...
    # *** for HK version *** //
    # Replace the joined mainland deflator with the Hong Kong deflator for Hong Kong rows only.
    GDP_2019_long <- GDP_2019_long %>%
      filter(province.name != "Hong Kong") %>%
      bind_rows(GDP_2019_long %>%
                  filter(province.name == "Hong Kong") %>%
                  select(-`GDP deflator`) %>%
    ...
    GDP_2019_long %>%
      mutate( "GDPperC_currentCNY" = (((( GDP / Sum ) * `National Total` ) * 100000000 ) / Population ) / 1000,
              # *** for HK version *** // Use Hong Kong's 2010 deflator base for Hong Kong rows.
              "GDPperC_constant2010CNY" = GDPperC_currentCNY * ( if_else(province.name != "Hong Kong", GDP_deflator_2010, GDP_deflator_2010_hk) / `GDP deflator` ),
    ...
    population_long_2024 %>%
      ...
      left_join( IMF_growth_rates_hk %>% select(Year, IMF_HK = `IMF growth rate`), by = "Year" ) %>% # *** for HK version *** // HK-specific short-term growth rate.
      mutate(`IMF growth rate` = if_else(province.name == "Hong Kong", IMF_HK, `IMF growth rate`)) %>% # *** for HK version *** //
      select(-IMF_HK) %>%
      mutate( "GDP_deflator_CNY_WB" = (( `National Total` / 10 ) *
                                                ( if_else(province.name != "Hong Kong", GDP_deflator_2010, GDP_deflator_2010_hk) / `GDP deflator` ) ) / CNY_USD_2010_exchange_rate ) -> GDP_deflator_CNY_WB_2024 # *** for HK version *** //
    ...
    GDP_final %>%
      ...
      add_precursors("gcam-china/GDP_raw",
                      ...
                      "gcam-china/GDP_deflator_HK", # *** for HK version *** // Hong Kong GDP deflator input.
                      "gcam-china/IMF_growth_rates",
                      "gcam-china/IMF_growth_rates_HK", # *** for HK version *** // Hong Kong IMF growth input.
