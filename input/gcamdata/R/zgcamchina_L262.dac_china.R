...
module_gcamchina_L262.dac <- function(command, ...) {
   ...
   if(command == driver.DECLARE_INPUTS) {
    ...
    dac_china_processing <- function(data, gcamchina.PROVINCES_NOMC) { # *** for HK version *** //
      ...
      if(nrow(check_df) == 0) {
        ...
        data %>%
          ...
          write_to_all_provinces(names = names(data), provinces = gcamchina.PROVINCES_NOMC) %>% # *** for HK version *** //
          # filter(region %in% gcamchina.PROVINCES_NOHKMC) ->
          # *** for HK version *** //
          filter(region %in% gcamchina.PROVINCES_NOMC) ->
          new_data
        # *** for HK version *** //
    ...
    L262.Supplysector_dac_china <- dac_china_processing(L262.Supplysector_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.FinalEnergyKeyword_dac_china <- dac_china_processing(L262.FinalEnergyKeyword_dac, gcamchina.PROVINCES_NOHKMC)
    L262.FinalEnergyKeyword_dac_china <- dac_china_processing(L262.FinalEnergyKeyword_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.SubsectorLogit_dac_china <- dac_china_processing(L262.SubsectorLogit_dac, gcamchina.PROVINCES_NOHKMC)
    L262.SubsectorLogit_dac_china <- dac_china_processing(L262.SubsectorLogit_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.SubsectorShrwtFllt_dac_china <- dac_china_processing(L262.SubsectorShrwtFllt_dac, gcamchina.PROVINCES_NOHKMC)
    L262.SubsectorShrwtFllt_dac_china <- dac_china_processing(L262.SubsectorShrwtFllt_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.SubsectorInterp_dac_china <- dac_china_processing(L262.SubsectorInterp_dac, gcamchina.PROVINCES_NOHKMC)
    L262.SubsectorInterp_dac_china <- dac_china_processing(L262.SubsectorInterp_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.StubTech_dac_china <- dac_china_processing(L262.StubTech_dac, gcamchina.PROVINCES_NOHKMC)
    L262.StubTech_dac_china <- dac_china_processing(L262.StubTech_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.PerCapitaBased_dac_china <- dac_china_processing(L262.PerCapitaBased_dac, gcamchina.PROVINCES_NOHKMC)
    L262.PerCapitaBased_dac_china <- dac_china_processing(L262.PerCapitaBased_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.PriceElasticity_dac_china <- dac_china_processing(L262.PriceElasticity_dac, gcamchina.PROVINCES_NOHKMC)
    L262.PriceElasticity_dac_china <- dac_china_processing(L262.PriceElasticity_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    # L262.CarbonCoef_dac_china <- dac_china_processing(L262.CarbonCoef_dac, gcamchina.PROVINCES_NOHKMC)
    L262.CarbonCoef_dac_china <- dac_china_processing(L262.CarbonCoef_dac, gcamchina.PROVINCES_NOMC) # *** for HK version *** //
    ...
    L262.GlobalTechCoef_dac_china <- dac_china_processing(L262.GlobalTechCoef_dac %>%
                                                        ...
                                                        mutate(region = gcamchina.REGION), gcamchina.PROVINCES_NOMC) # *** for HK version *** //
